...
def loadResult(filename):
    ...
    if not isfile(ANSWER_PATH+filename):
        return
    if not isfile(OPENCFU_PATH+"opencfu_"+basename(filename[:-8])+".csv"):
        return
...
#cv2.circle(img,(x,y),5,colorList[group],-1)
    ...
    f2 = open(OPENCFU_PATH+"opencfu_"+basename(filename[:-8])+".csv", 'r+')
    f2.readline()
    opencfu_resultList = []
    opencfu_count = 0
    for line in f2:
        opencfu_count += 1
        Full_Path, IsValid, x, y, ROI, group, N_in_clust, Area, Radius, Hue, Saturation, Rmean, Gmean, Bmean, Rs, Gs, Bs, comment = line.split(",")
        x = int(float(x))
        y = int(float(y))
        group = int(group)
#cv2.circle(img,(x,y),5,colorList[group],-1)
        opencfu_resultList.append([group, y, x, False])
    f2.close

    return resultList, opencfu_resultList

def saveResult(resultList):
    f = open("opencfu_result.csv", 'w+')
...
if __name__=="__main__":
    ...
    for root, dirs, files in os.walk(ANSWER_PATH):
        for filename in files:
            ...
            answerList, opencfu_resultList = loadResult(filename)

            print len(opencfu_resultList)

            if answerList != None:
                falsePositive = len(opencfu_resultList)
                truePositive  = 0
                falseNegative = len(answerList)

            for opencfuResult in opencfu_resultList:
                center_y = opencfuResult[1]
                center_x = opencfuResult[2]
            ...
            if answerList != None:
                ...
                error_rate = countErrorRate(len(answerList),len(opencfu_resultList))
